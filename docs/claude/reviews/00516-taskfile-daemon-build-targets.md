...
> **Before you start:**
>
> - On Apple Silicon (M-series) Macs the UTM VM is **arm64**. Substitute
>   `solo-provisioner-linux-arm64` for `solo-provisioner-linux-amd64` in
>   every command below — `task build` produces both arches.
> - `--version` and the `version` subcommand currently trigger the install
>   pre-run check (see issue #615). Steps that invoke them on an
>   uninstalled binary will fail; install first
>   (`./bin/solo-provisioner-linux-arm64 install --config <config>`), or —
>   if you only need to verify the embedded value — run
>   `cat pkg/version/cli/VERSION` and `cat pkg/version/daemon/VERSION`
>   directly.
> - `pkg/version/{cli,daemon}/{VERSION,COMMIT}` are gitignored and
>   regenerated by `task generate` (a dep of `task build`). Any step
>   below that edits one of those files assumes `task build` (or
>   `task generate`) has run at least once so the file exists.
