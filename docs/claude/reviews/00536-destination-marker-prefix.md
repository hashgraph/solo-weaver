...
| File | What changed |
|---|---|
| `pkg/manifests/external_files.go` | Added unexported `allowedDestinationPrefixes` slice (enforcement source of truth), exported `AllowedDestinationPrefixes()` accessor returning a cloned slice, and `validateDestinationPrefix(field, dest)` helper; wired into `ExternalFile.validate` after the non-empty check. New imports: `slices`, `strings` |
| `pkg/manifests/external_files_test.go` | Removed `TestParseExternalFiles_DestinationPrefixNotYetEnforced`; added `TestParseExternalFiles_DestinationPrefixEnforcement` (3 accept + 5 reject subtests), `TestAllowedDestinationPrefixes_PinnedSet`, and `TestAllowedDestinationPrefixes_ReturnedSliceIsClone` |
| `docs/claude/reviews/00536-destination-marker-prefix.md` (new) | This guide |

## Code review checklist

- [ ] **`AllowedDestinationPrefixes()` is exported and returns a clone.** External tooling (e.g. CN deployment-package linters) can call it to reuse the closed set without forking the list. The exported accessor wraps an unexported `allowedDestinationPrefixes` slice (the source of truth read by `validateDestinationPrefix`) so callers cannot weaken enforcement by mutating the returned slice — `TestAllowedDestinationPrefixes_ReturnedSliceIsClone` pins that contract.
