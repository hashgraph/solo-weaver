...
// nftUnitProbeTimeout bounds the DBus probe in Applies, which runs before every
// root invocation; without it a stuck systemd would hang every command.
const nftUnitProbeTimeout = 5 * time.Second
...
func (m *NetworkNftUnitMigration) Applies(mctx *migration.Context) (bool, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), nftUnitProbeTimeout)
	defer cancel()
	needsConverge, err := networkNftUnitNeedsConverge(ctx)
	if err != nil {
