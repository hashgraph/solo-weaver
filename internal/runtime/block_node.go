		...
		automa.WithEffectiveFunc(
			func(
				...
				if userInput != nil && userInput.Val() != "" {
					...
					// if cluster is already created, use the current namespace
					if current.Created && current.Namespace != userInput.Val() {
		...
		automa.WithEffectiveFunc(
			func(
				...
				if userInput != nil {
					...
					if current.Created && !reflect.DeepEqual(current.Storage, userInput.Val()) {
						val = current.Storage
						strategy = automa.StrategyCurrent
					}
		...
		automa.WithEffectiveFunc(
			func(
				...
				if userInput != nil && userInput.Val() != "" {
					...
					if current.Created && current.Version != userInput.Val() {
						val = current.Version
						strategy = automa.StrategyCurrent
					}
		...
		automa.WithEffectiveFunc(
			func(
				...
				if userInput != nil && userInput.Val() != "" {
					...
					if current.Created && current.Release != userInput.Val() {
						val = current.Release
						strategy = automa.StrategyCurrent
					}
		...
		automa.WithEffectiveFunc(
			func(
				...
				if userInput != nil && userInput.Val() != "" {
					...
					if current.Created && current.Chart != userInput.Val() {
						val = current.Chart
						strategy = automa.StrategyCurrent
					}
		...
		automa.WithEffectiveFunc(
			func(
				...
				if userInput != nil && userInput.Val() != "" {
					...
					if current.Created && current.ChartVersion != userInput.Val() {
						val = current.ChartVersion
						strategy = automa.StrategyCurrent
					}
...
func InitBlockNodeRuntime(state core.BlockNodeState, realityChecker reality.Checker, refreshInterval time.Duration) error {
	...
	rb := NewRuntimeBase[core.BlockNodeState](
		...
		// lastSync extractor
		func(s *core.BlockNodeState) *time.Time { return s.LastSync },
		// clone helper
