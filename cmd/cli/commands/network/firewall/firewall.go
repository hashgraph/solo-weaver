...
func init() {
	firewallCmd.AddCommand(createCmd, createAllowRuleCmd, addCmd, removeCmd, setCmd, showCmd, reapplyCmd, refreshDNSCmd, deleteCmd)
}
...
func registerTargetFlags(cmd *cobra.Command, verb string) {
	...
	cmd.Flags().StringSliceVar(&flagCIDRs, "cidr", nil,
		"CIDR(s) or, on every rule but in_cluster, domain name(s) to "+verb+" (comma-separated or repeated)")
