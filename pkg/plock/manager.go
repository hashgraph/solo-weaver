...
func newLockManagerWithStore(store fileStore) (*plockManager, error) {
	if store == nil {
		return nil, errorx.IllegalArgument.New("file store cannot be nil: %s", store)
	}
...
func (pm *plockManager) Discover(maxCount int) (map[int]*Info, error) {
	...
	if err != nil {
		return nil, errorx.IllegalState.New("Could not read file list from work directory: %s", pm.GetWorkDir()).
...
func (pm *plockManager) DiscoverByLockName(lockName string) (*Info, error) {
	...
	if err != nil {
		return nil, errorx.IllegalArgument.New("Could not read file list from work directory: %s", pm.GetWorkDir()).
			WithUnderlyingErrors(err)
	}
	...
	return nil, errorx.IllegalArgument.New("Could not find lock with name: %s", lockName)
}

func (pm *plockManager) ResetStaleLock(info Info) error {
	if !isStalePID(info.PID) {
		return errorx.IllegalArgument.New("Lock's PID is not stale and cannot be reset: %s", strconv.Itoa(info.PID))
	}
	...
	if pm.store.Exists(pidFileName) {
		...
		if err != nil {
			return errorx.IllegalState.New("Could not remove pid file: %s", info.PidFilePath).WithUnderlyingErrors(err)
		}
	}
	...
	if pm.store.Exists(lockFileName) {
		...
		if err != nil {
			return errorx.IllegalState.New("Could not remove lock file: %s", info.LockFilePath).WithUnderlyingErrors(err)
		}
	}
...
func (pm *plockManager) pidFileToLockInfo(pidFilePath string) (*Info, error) {
	...
	if err != nil {
		return nil, errorx.IllegalArgument.New("Cannot parse PidFilePath: %s", pidFilePath).
			WithUnderlyingErrors(err)
	}
	...
	return nil, errorx.IllegalArgument.New("invalid pid lock file path: %s", pidFilePath)
}
