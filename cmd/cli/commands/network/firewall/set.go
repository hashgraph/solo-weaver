...
func init() {
	...
	setCmd.Flags().StringSliceVar(&flagCIDRs, "cidrs", nil, "Full CIDR (or, on every rule but in_cluster, domain name) list for --name (comma-separated; replaces the existing list)")
	...
	setCmd.Flags().StringSliceVar(&flagBlockedCIDRs, "blocked-cidrs", nil, "Full operator block list: IPv4 CIDRs and/or FQDNs (comma-separated; replaces the existing list)")
