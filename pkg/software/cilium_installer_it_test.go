...
func Test_CiliumInstaller_FullWorkflow_Success(t *testing.T) {
	...
	extractedFiles, err := os.ReadDir("/opt/solo/weaver/tmp/cilium/unpack")
	require.NoError(t, err)
	...
	_, exists, err := fileManager.PathExists("/opt/solo/weaver/sandbox/bin/cilium")
	require.NoError(t, err)
	...
	_, exists, err = fileManager.PathExists("/opt/solo/weaver/tmp/cilium")
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, "/opt/solo/weaver/sandbox/bin/cilium", linkTarget, "symlink should point to sandbox binary")

	// Verify /opt/solo/weaver/sandbox/etc/weaver/cilium-config.yaml contains valid ip address and sandbox folder
	configFilePath := "/opt/solo/weaver/sandbox/etc/weaver/cilium-config.yaml"
	...
	require.Regexp(t, regexp.MustCompile(`binPath: /opt/solo/weaver/sandbox/opt/cni/bin`), string(configContent), "cilium configuration file should contain correct binPath")
	require.Regexp(t, regexp.MustCompile(`confPath: /opt/solo/weaver/sandbox/etc/cni/net.d`), string(configContent), "cilium configuration file should contain correct confPath")
	require.Regexp(t, regexp.MustCompile(`runPath: /opt/solo/weaver/sandbox/var/run/cilium`), string(configContent), "cilium configuration file should contain correct runPath")
