...
- **Every `automa.StepFailureReport` must include a `WithProperty(models.ErrPropertyResolution, []string{...})` block** listing operator-actionable steps to resolve the failure (e.g. commands to run, paths to check). This is surfaced in the TUI and in error output to the operator. A failure report without resolution hints is incomplete — treat it the same as a missing error message.
