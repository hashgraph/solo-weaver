...
// CLI verb. It renders the boot-replay script (solo-provisioner-bandwidth-shaper.sh)
// with bandwidth-parameterised class configurations, manages the
// solo-provisioner-bandwidth-shaper.service oneshot unit, and applies live tc class
...
const (
	...
	// tcEgressScriptTemplate is the embedded Go template for the bandwidth-shaper.sh
	// boot script. Interpolation points: {{.NIC}}, {{.Device.*}}, {{range .Classes}}.
	tcEgressScriptTemplate = "files/network/solo-provisioner-bandwidth-shaper.sh.tmpl"
