...
// keys exposed by github.com/automa-saga/version. Values are not asserted because
// they are stamped via -ldflags at build time and default to dev/none under `go test`.
...
// TestRenderVersion_YAML verifies the yaml branch (marshaled in-repo since the
// upstream Format only does json/text) emits valid YAML with the metadata keys.
...
// with an errorx.IllegalFormat error (preserving the prior --output contract of
// json/yaml only).
