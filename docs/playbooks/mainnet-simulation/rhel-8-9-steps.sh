...
if [[ -f /etc/os-release ]]; then
  ...
  if [[ "${ID}" == "rhel" && "${VERSION_ID}" =~ ^8\.[0-9]+ ]]; then
    RHEL8_DETECTED=true
  fi
fi
...
sudo dnf remove -y containerd containerd.io
...
sudo dnf install -y conntrack socat ebtables nftables
...
#sudo sed -i 's/^GRUB_CMDLINE_LINUX="\(.*\)"$/GRUB_CMDLINE_LINUX="systemd.unified_cgroup_hierarchy=1 \1"/' /etc/default/grub
