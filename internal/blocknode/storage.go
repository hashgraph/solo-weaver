...
func (m *Manager) GetStoragePaths() (archivePath, livePath, logPath string, optionalPaths []string, err error) {
	return ResolveStoragePaths(m.blockNodeInputs.Storage, m.blockNodeInputs.ChartVersion)
}

// ResolveStoragePaths is the pure-logic counterpart of Manager.GetStoragePaths:
// it derives the effective archive/live/log/optional storage paths from a
// BlockNodeStorage config and a chart version, without requiring a Manager
// (and therefore without constructing fsx/helm/kube clients). Use this when
// you only need to canonicalize or compare paths.
func ResolveStoragePaths(storage models.BlockNodeStorage, chartVersion string) (archivePath, livePath, logPath string, optionalPaths []string, err error) {
	archivePath = storage.ArchivePath
	livePath = storage.LivePath
	logPath = storage.LogPath

	applicable := GetApplicableOptionalStorages(chartVersion)
