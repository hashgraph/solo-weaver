...
var refreshDNSCmd = &cobra.Command{
	Use:   "refresh-dns",
	Short: "Re-resolve the management allowlist's domain names and apply any address changes",
	Long: "Re-resolve every fully-qualified domain name in the `mgmt` rule of " + fw.HostConfigPath + " and, if any " +
		"of them now point somewhere else, re-render and re-apply the table.\n\n" +
		...
		"never narrows the allowlist. A name that has never resolved contributes nothing and is logged. If that " +
		"would leave the allowlist empty the command fails instead, because an empty @mgmt_addrs under the input " +
		"chain's default drop would take every new SSH connection with it.\n\n" +
		"Nothing to configure: the names come from the persisted config, and only the `mgmt` rule accepts them.",
	RunE: func(cmd *cobra.Command, _ []string) error {
		if err := newManager().RefreshDNS(cmd.Context()); err != nil {
			return err
		}
		logx.As().Info().Msg("management allowlist domain names re-resolved")
		return nil
	},
}
