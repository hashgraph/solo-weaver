...
the digest document `privexec.ReconcileShaperCheck` parses back; log lines go to stderr, so a
stray log no longer corrupts that decode — but it still reaches nobody, because nothing reads the
daemon-scheduled worker's stderr. Every name needing attention is returned instead, on three
separate lists — `Unresolved`, `Stale`, `AAAAOnly`, each a `(name, policies)` pair — surfacing on
`CheckResult` and `Result`. The command layer prints them on the human-readable path only; the
daemon logs them from the `--output json` payload it already parses for both the `--check` probe
and the privileged apply, which is the only way any of the three reach an operator on the
