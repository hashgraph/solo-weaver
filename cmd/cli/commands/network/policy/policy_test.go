...
func newTestEnv(t *testing.T) *testEnv {
	t.Helper()
	dir := t.TempDir()
	return &testEnv{dir: dir, nftPath: filepath.Join(dir, "network-weaver-workload-policy.nft"), runner: newFakeRunner()}
}

// runCreate executes the real create command against this env's manager and
// stubbed pod-CIDR detection, returning the persisted network-weaver-workload-policy.nft.
...
func TestDeleteCmd_RemovesPolicy(t *testing.T) {
	...
	// and the live inet weaver-workload-policy table is deleted.
	require.NoFileExists(t, env.nftPath)
	require.Empty(t, env.runner.applied, "live inet weaver-workload-policy table must be torn down after the last policy is deleted")
}
