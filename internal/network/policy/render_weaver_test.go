...
func TestRenderWeaverNft_EmptyRegistryRemovesStaleFile(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(out, []byte("table inet weaver-workload-policy { }\n"), 0o644))

	require.NoError(t, RenderWeaverNft(filepath.Join(dir, "policies"), out, ""))

	_, err := os.Stat(out)
	require.True(t, os.IsNotExist(err), "stale network-weaver-workload-policy.nft must be removed for an empty registry")
}
