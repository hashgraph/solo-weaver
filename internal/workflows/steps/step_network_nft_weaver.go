...
// NftWeaverPersist re-renders /etc/solo-provisioner/network-weaver-workload-policy.nft from
// the policy registry and restarts the shared solo-provisioner-network-nft.service
// oneshot so the kernel loads both network-weaver-host-firewall.nft and network-weaver-workload-policy.nft via
...
func NftWeaverPersist() *automa.StepBuilder {
	return automa.NewStepBuilder().WithId(NftWeaverPersistStepId).
		...
		WithExecute(func(ctx context.Context, stp automa.Step) *automa.Report {
			...
			if empty {
				return automa.SkippedReport(stp,
					automa.WithDetail("policy registry is empty; ensured no network-weaver-workload-policy.nft is persisted — weaver rules are written after `network policy create` runs"))
			}
		...
		WithRollback(func(ctx context.Context, stp automa.Step) *automa.Report {
			...
			// idempotent artifacts. Removing network-weaver-workload-policy.nft on rollback would
			// leave the live inet weaver-workload-policy table present but un-replayed on next
