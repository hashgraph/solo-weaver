...
- **The daemon is built before the CLI, and its SHA-256 is stamped into the CLI at link time** (`-X …/pkg/software.pinnedDaemonDigest`, see `build:cli` in `taskfiles/cli.yaml`). That digest is what the CLI's daemon auto-download verifies against, so `build:cli` declares `build:daemon` as a dep and `task build` calls only `build:cli:all`. `task sign` ends with `verify:cli:all`, which fails the release if any CLI does not carry the digest of the daemon beside it — never publish a CLI/daemon pair that has not passed it
