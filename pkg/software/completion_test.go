...
// installFakeTools reproduces the /usr/local/bin -> sandbox symlink layout under
// one temp dir. kind selects the shapes weaverManagedTool must tell apart.
func installFakeTools(t *testing.T, kind string, cmds ...string) {
	t.Helper()
...
func Test_WriteBashCompletionLoader(t *testing.T) {
	...
	mockFsx.EXPECT().PathExists(bashCompletionPath("kubectl")).Return(nil, false, nil)
...
func Test_WriteBashCompletionLoader_Errors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expects: func(m *fsx.MockManager) {
				m.EXPECT().PathExists(gomock.Any()).Return(nil, false, nil)
				m.EXPECT().CreateDirectory(bashCompletionDir, true).
					Return(errors.New("read-only file system"))
			},
		},
		{
			name: "existence probe failure is reported",
			expects: func(m *fsx.MockManager) {
				m.EXPECT().PathExists(gomock.Any()).
					Return(nil, false, errors.New("not a directory"))
				// No CreateDirectory/AtomicWriteFile: we do not write what we
				// could not check for.
			},
		},
		{
...
func Test_WriteBashCompletionLoader_LeavesForeignFile(t *testing.T) {
	...
	mockFsx := fsx.NewMockManager(ctrl)
	mockFsx.EXPECT().PathExists(bashCompletionPath("kubectl")).Return(nil, true, nil)
...
// A loader that exists but cannot be read is somebody else's as far as we can
// tell, so it must not be replaced.
func Test_WriteBashCompletionLoader_LeavesUnreadableFile(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	// No AtomicWriteFile: an unidentifiable file is left alone.
...
// A failure on one tool must not stop the other from being attempted.
func Test_ReconfigureShellCompletion_ContinuesAfterFailure(t *testing.T) {
	reRootCompletionDir(t)
	installFakeTools(t, "weaver", KubectlBinaryName, HelmBinaryName)

	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	mockFsx := fsx.NewMockManager(ctrl)
	mockFsx.EXPECT().PathExists(bashCompletionPath(KubectlBinaryName)).
		Return(nil, false, errors.New("probe failed"))
	// helm is still written; ctrl.Finish fails if these go uncalled.
	mockFsx.EXPECT().PathExists(bashCompletionPath(HelmBinaryName)).Return(nil, false, nil)
	mockFsx.EXPECT().CreateDirectory(bashCompletionDir, true).Return(nil)
	mockFsx.EXPECT().AtomicWriteFile(bashCompletionPath(HelmBinaryName),
		[]byte(bashCompletionLoader), os.FileMode(models.DefaultFilePerm)).Return(nil)

	require.Error(t, reconfigureShellCompletion(mockFsx))
}

func Test_RemoveShellCompletion_ContinuesAfterFailure(t *testing.T) {
	reRootCompletionDir(t)
	...
	for _, cmd := range completionCommands {
		mockFsx.EXPECT().ReadFile(bashCompletionPath(cmd), int64(-1)).
			Return([]byte(bashCompletionLoader), nil)
	}
	mockFsx.EXPECT().RemoveAll(bashCompletionPath(KubectlBinaryName)).
		Return(errors.New("device busy"))
	mockFsx.EXPECT().RemoveAll(bashCompletionPath(HelmBinaryName)).Return(nil)

	require.Error(t, removeShellCompletion(mockFsx))
}
