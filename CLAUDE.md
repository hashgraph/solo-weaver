...
- Builds produce two binaries at `bin/solo-provisioner-{OS}-{ARCH}` (CLI) and `bin/solo-provisioner-daemon-{OS}-{ARCH}` (daemon); `task build` produces both, and `task hash`/`task sign` cover both
