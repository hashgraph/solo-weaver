...
// loaderPresent reports whether anything occupies the loader path. It does not
// follow symlinks: the write path leaves alone whatever it cannot identify, so
// treating a dangling link as absent would have callers ask for a write that
// never happens.
func loaderPresent(cmd string) bool {
	_, err := os.Lstat(bashCompletionPath(cmd))
...
func writeBashCompletionLoader(fm fsx.Manager, cmd string) error {
	...
	_, exists, err := fm.PathExists(loaderPath)
	if err != nil {
		return NewConfigurationError(err, cmd)
	}

	if exists && !weaverWroteLoader(fm, loaderPath) {
...
func removeBashCompletionLoader(fm fsx.Manager, cmd string) error {
	...
	if !weaverWroteLoader(fm, loaderPath) {
		// Absent, unreadable, or somebody else's: nothing for us to remove.
		return nil
	}
...
// weaverWroteLoader reports whether the file at path is the loader weaver
// writes. A file that cannot be read counts as somebody else's: replacing or
// deleting what we cannot identify would break the guarantee that
// operator-managed files are left alone.
func weaverWroteLoader(fm fsx.Manager, path string) bool {
	content, err := fm.ReadFile(path, -1)

	return err == nil && string(content) == bashCompletionLoader
}
